...
class ControllerPipeTest extends AbstractPipeTestCase {
	...
	protected function performControllerExcecution(ControllerPipe $instance, $controllerClassName) {
		$controllerMock = $this->getMockForAbstractClass('FluidTYPO3\Flux\Controller\AbstractFluxController', [], $controllerClassName, TRUE, TRUE, TRUE,
			[
		...
		$signalSlotDispatcherMock = $this->getMock('TYPO3\CMS\Extbase\SignalSlot\Dispatcher', ['dispatch']);
		$configurationManagerMock = $this->getMock('TYPO3\CMS\Extbase\Configuration\ConfigurationManager', ['isFeatureEnabled']);
		$configurationManagerMock->expects($this->any())->method('isFeatureEnabled')->will($this->returnValue(TRUE));
		$propertyMappingServiceMock = $this->getMock('TYPO3\CMS\Extbase\Mvc\Controller\MvcPropertyMappingConfigurationService', ['initializePropertyMappingConfigurationFromRequest']);
		$argumentsMock = $this->getMock('TYPO3\CMS\Extbase\Mvc\Controller\Arguments', ['getIterator']);
		$argumentsMock->expects($this->atLeastOnce())->method('getIterator')->will($this->returnValue(new \ArrayIterator([new Argument('test', 'string')])));
		...
		$objectManagerMock = $this->getMock('TYPO3\CMS\Extbase\Object\ObjectManager', ['get']);
		$response = $this->getMock('TYPO3\CMS\Extbase\Mvc\Web\Response', ['getContent']);
		$response->expects($this->once())->method('getContent')->will($this->returnValue($this->defaultData));
		$request = $this->getMock('TYPO3\CMS\Extbase\Mvc\Web\Request', ['getControllerActionName', 'getMethodParameters', 'getDispatched']);
		...
		$dispatcherMock = $this->getMock('TYPO3\CMS\Extbase\Mvc\Dispatcher', ['resolveController'], [$objectManagerMock]);
