...
abstract class AbstractApplicator {
	...
	public function initialize() {
		$cacheFile = GeneralUtility::getFileAbsFileName('typo3temp/Cache/Data/flux/packages');
		if (FALSE === file_exists($cacheFile)) {
			return;
		}
		$packages = unserialize(file_get_contents($cacheFile));
		$this->initializePlugins($packages);
	}
	 ...
	 * @return void
	 */
	protected function initializePlugins(Collection $packages) {
		foreach ($packages->getAll() as $package) {
			/** @var PackageInterface $package */
			foreach ($package->getPlugins()->getAll() as $plugin) {
				/** @var PluginDefinition $plugin */
				$this->applyPluginDefinition($package, $plugin);
			}
		}
	}
